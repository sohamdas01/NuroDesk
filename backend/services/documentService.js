...
//       console.log('⚠️ YOUTUBE_COOKIES not set in production - downloads may fail');
//     }
//     return null;
//   }
  
//   try {
//     let cleanedContent = cookiesContent
//       .trim()
//       .replace(/\r\n/g, '\n')
//       .replace(/\n+/g, '\n');
    
//     if (!cleanedContent.includes('# Netscape HTTP Cookie File')) {
//       cleanedContent = '# Netscape HTTP Cookie File\n# http://curl.haxx.se/rfc/cookie_spec.html\n' + cleanedContent;
//     }
    
//     const cookiePath = path.join(process.cwd(), 'temp_cookies.txt');
//     fs.writeFileSync(cookiePath, cleanedContent);
    
//     console.log(`✅ Cookies file created: ${fs.statSync(cookiePath).size} bytes`);
//     return cookiePath;
...
async function fetchYouTubeContent(videoId) {
  ...
  try {
    ...
    // Try GitHub code from description
    let githubCode = null;
    if (metadata.description) {
      githubCode = await fetchGitHubCode(metadata.description);
    }

    // ✅ CAPTION-ONLY MODE - No downloads!
    console.log('🎯 Attempting to fetch captions...');
    ...
    if (!captionText || captionText.length < 100) {
      throw new Error(
        ...
        `   • News channel videos (usually have captions)\n` +
        `   • Official music videos (often have captions)\n\n` +
        `💡 TIP: Look for the [CC] icon on YouTube videos!\n\n` +
        `Video: ${metadata.title || videoUrl}`
      );
    }

    console.log(`✅ SUCCESS: Using captions (${captionText.length} characters)\n`);

    // Build content with metadata + captions
    let fullContent = '';
